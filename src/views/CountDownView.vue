<template>
  <div class="count-down-view__wrapper" :class="getThemeClass()">
    <div class="todolist">
      <todo-creator />
      <div class="count-down" v-if="doing">
        <todo-list :todoList="[doing]" showTomatoes :tomatoes="tomatoes" @onCheckTodo="resetCountDown"/>
        <div class="count-down-time">{{ formatedRemainTime }}</div>
      </div>
      <p v-else>沒有 Todo, 新增一個吧!</p>
      <div class="bottom">
    ...
    <div class="clock-controller" :class="getClockStatusClass()">
      <button class="count-down-btn" @click="startCountDown" :style="`background: conic-gradient(${getProgressbar()}, transparent 0);`">
        <div class="count-down-btn-icon">
          <icon-pause v-if="isContinue" />
          <icon-play v-else />
        </div>
        <button @click.stop="resetCountDown" class="reset-btn"></button>
      </button>
    </div>
  </div>
</template>
